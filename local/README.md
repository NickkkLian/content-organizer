...
## 用法：双击「开关」App（不开终端）
**双击 `收藏整理库抓取.app`** —— 它**先弹窗告诉你当前状态**，再由你按按钮（不是盲切换）：
- 弹「⚪ 抓取服务：已停止」→ 点 **「启动服务」**：启动 + **口令自动复制到剪贴板**；点「先不启动」则什么都不做。
- 弹「🟢 抓取服务：运行中」→ 点 **「停止服务」** 停掉；点「保持运行」则什么都不做。
- 弹窗 15 秒没人点会自动消失，**什么都不做**（不会误启动/误停止）。
- 首次要绑口令：打开网页库 [收藏整理库](https://nickkklian.github.io/content-organizer/) → ⚙️ 设置 → 「本地抓视频口令」粘上 → 保存（**只需一次**，之后浏览器记住）。
- 之后在「🎬 抓视频」粘链接即可。

> ⚠️ **改了 `content_server.py` 要重启服务才生效**（跑着的进程还是启动那一刻的代码）：双击 → 停止服务 → 再双击 → 启动服务。
> 开关跑的就是**本目录**这份 `content_server.py`（路径写死在 `co_toggle.applescript` 的 `localDir`），所以这个镜像目录 = 实际运行时，别让它落后于线上仓库。
